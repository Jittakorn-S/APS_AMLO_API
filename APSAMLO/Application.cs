    ...
    public class Application
    {
        ...
        public async Task<ApplicationRunResponse> Run()
        {
            ...
            try
            {
                ...
                ConfigModel.Instance.APIKey = configurationInterface?["AppSettings:x-api-key"];
                ConfigModel.Instance.CertificatePassword = configurationInterface?["AppSettings:certificatepassword"];
                ConfigModel.Instance.CertificatePath = configurationInterface?["AppSettings:certificate"];
                ConfigModel.Instance.ProcessPath = configurationInterface?["AppSettings:processpath"];
                ConfigModel.Instance.KeyFile = configurationInterface?["AppSettings:keyfile"];
                ConfigModel.Instance.PassPhrase = configurationInterface?["AppSettings:passphrase"];
                ConfigModel.Instance.PassWord = configurationInterface?["AppSettings:password"];
                ConfigModel.Instance.UserName = configurationInterface?["AppSettings:username"];
                ConfigModel.Instance.ZipFilePath = configurationInterface?["AppSettings:zipfilepath"];
                ConfigModel.Instance.BackupPath = configurationInterface?["AppSettings:backuppath"];
        ...
        public static async Task<Message?> CheckDataVersion()
        {
            ...
            if (!Directory.Exists(ConfigModel.Instance.ProcessPath))
            {
                Directory.CreateDirectory(ConfigModel.Instance.ProcessPath);
            }

            if (!Directory.Exists(ConfigModel.Instance.ZipFilePath))
            {
                Directory.CreateDirectory(ConfigModel.Instance.ZipFilePath);
            }

            if (!Directory.Exists(ConfigModel.Instance.BackupPath))
            {
                Directory.CreateDirectory(ConfigModel.Instance.BackupPath);
            }

            string[] filesInzip = Directory.GetFiles(ConfigModel.Instance.ZipFilePath);
            string[] filesInbackup = Directory.GetFiles(ConfigModel.Instance.BackupPath);
            ...
            var certificate = new X509Certificate2(ConfigModel.Instance.CertificatePath, ConfigModel.Instance.CertificatePassword);
            ...
            try
            {
                ...
                {
                    string base64Credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{ConfigModel.Instance.UserName}:{ConfigModel.Instance.PassWord}"));
                    string versionFilePath = Path.Combine(ConfigModel.Instance.ProcessPath, "Version.txt");
                    ...
                    httpClient.DefaultRequestHeaders.Add("x-api-key", ConfigModel.Instance.APIKey);
        ...
        private static async Task GetDataDPRK()
        {
                ...
                using (var response = await httpClient.PostAsync(apiFreeze05Path, null))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        try
                        {
                            ...
                            string PGPfilePath = Path.Combine(ConfigModel.Instance.ProcessPath, outputFileName);
                            OutputFilePath = Path.Combine(ConfigModel.Instance.ZipFilePath, outputFileName.Replace(".pgp", ""));

                            File.WriteAllText(PGPfilePath, PGPData);

                            using (ChoPGPEncryptDecrypt pgp = new ChoPGPEncryptDecrypt())
                            {
                                pgp.DecryptFile(PGPfilePath, OutputFilePath, ConfigModel.Instance.KeyFile, ConfigModel.Instance.PassPhrase);
            ...
            {
                File.Copy(OutputFilePath, ConfigModel.Instance.BackupPath + @"\" + fileName, true);
